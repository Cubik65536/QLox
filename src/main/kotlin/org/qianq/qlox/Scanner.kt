        ...
        when (val c = nextChar()) {
            ...
            '/' -> {
                if (match('/')) {
                    ...
                    while (peek() != '\n' && !isAtEnd()) nextChar()
                    QLox.log(line, "COMMENT\t\t${src.substring(start, current)}")
                } else if (match('*')) {
                    // Start a block comment
                    while ((peek() != '*' || peekNext() != '/') && !isAtEnd()) {
                        /*
                         * It is not at the end of src
                         * end of block comment not peeked
                         * continue to consume characters
                         */
                        if (peek() == '\n') line++
                        nextChar()
                    }
                    if (match('*') && match('/')) {
                        // End a block comment
                        QLox.log(line, "BLOCK COMMENT\t${src.substring(start, current)}")
                    } else {
